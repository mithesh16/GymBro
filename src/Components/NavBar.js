...
const NavBar = () => {
  ...
  return (
    ...
    <div className='flex items-center flex-shrink-0 text-white mr-10 '>
      <Link to='/home'>
        <span className="font-semibold text-4xl tracking-tight text-violet-400 font-['Futura']">GYMBROS</span>
      </Link>
    </div>
  ...
  <div className={`hidden lg:flex items-center justify-center text-sm px-3 py-1 leading-none border rounded-3xl text-white border-white lg:mt-0`}>
    ...
    <div role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
      ...
      <button
  class="absolute top-20 right-8  mt-3 w-56 origin-top-right inline-block rounded border border-current px-8 py-3 bg-gray-900 text-sm font-medium text-violet-400 transition hover:scale-110 hover:shadow-xl hover:border-violet-200 hover:ring-violet-200 focus:outline-none focus:ring active:text-violet-400"
  href="#"
>
  Sign Out
</button>
      ...
      <div onClick={handleNav} className='block lg:hidden z-[1001] '>
        {nav ? <AiOutlineClose size={30} style={{ fill: 'black', stroke: 'black', backgroundColor: 'white', borderRadius: '50%',padding:'3px',right:'10px' }} /> : <AiOutlineMenu size={30} />}
      </div>
      ...
      <ul
        className={
          nav
            ? 'fixed lg:hidden left-0 top-0  w-[75%] h-full border-r border-r-gray-900 bg-gray-900 ease-in-out duration-300 z-[1000] '
      ...
      <Link to='/home'>
        ...
        <div className=' inline-flex items-center justify-center text-sm py-2 px-4 leading-none border rounded-3xl mt-10 text-white border-white lg:mt-0 lg:hidden w-fit max-w-[90%] flex-wrap'>
    <img src={user.photoURL} className='rounded-3xl w-9  h-9 mr-2 border-white'/>
